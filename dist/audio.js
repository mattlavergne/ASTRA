import {dbGain,hz,melodic,stepSeconds,stepTime,eventsAt,arrangementSteps,locateStep} from './model.js';
...
export function drumPCM(kind,rate=48000,tune=0){
 const duration={kick:0.85,snare:0.32,hat:0.11,openhat:0.65,clap:0.3,rim:0.12,tom:0.6,conga:0.42,shaker:0.14,cowbell:0.38,ride:0.95,crash:1.6}[kind]||0.3;
 ...
 if(kind==='conga'){phase+=2*Math.PI*(210+180*Math.exp(-t*55))*pitch/rate;v=(Math.sin(phase)*0.86+high*Math.exp(-t*120)*0.3)*Math.exp(-t*13);}
 if(kind==='shaker'){const swell=Math.min(1,t/0.008);v=high*swell*Math.exp(-t*46)*0.62;}
 if(kind==='cowbell'){const a=Math.sin(2*Math.PI*542*pitch*t),b=Math.sin(2*Math.PI*832*pitch*t);v=(Math.sign(a)*0.42+Math.sign(b)*0.34)*Math.exp(-t*15)*0.8;}
 if(kind==='ride'||kind==='crash'){const wash=[2760,3730,4830,5960,7120,8410].reduce((s,f)=>s+Math.sin(2*Math.PI*f*pitch*t+Math.sin(2*Math.PI*f*0.73*pitch*t)),0)/6;
  const decay=kind==='ride'?4.6:2.1,ping=kind==='ride'?Math.sin(2*Math.PI*1180*pitch*t)*Math.exp(-t*26)*0.38:0;
  v=(high*0.42+wash*0.3)*Math.exp(-t*decay)+ping;}
...
// Oscillator recipes per melodic voice: [frequency ratio, waveform, level]. Sustain is the level
// the note holds at after its decay, which is what separates a pluck from an organ.
const LAYERS={keys:[[1,'sine',0.7],[2,'sine',0.2],[3.01,'sine',0.12]],pad:[[1,'sawtooth',0.4],[1.004,'sawtooth',0.4],[0.5,'sine',0.2]],
 bass:[[1,'sine',0.85],[2,'triangle',0.18]],lead:[[1,'sawtooth',0.42],[1.003,'triangle',0.48]],
 pluck:[[1,'triangle',0.6],[2,'sawtooth',0.22],[3.02,'sine',0.1]],
 bell:[[1,'sine',0.5],[2.76,'sine',0.26],[5.4,'sine',0.14],[8.93,'sine',0.06]],
 organ:[[1,'sine',0.46],[2,'sine',0.3],[3,'sine',0.18],[4,'sine',0.12],[8,'sine',0.06]]};
const SUSTAIN={keys:0.1,pad:0.3,bass:0.3,lead:0.3,pluck:0.04,bell:0.08,organ:0.62,sample:0.3};
...
export class AudioGraph{
 ...
 const attack=Math.min(t.attack,duration*0.5),sustain=Math.max(0.001,v*(SUSTAIN[t.instrument]??0.3));gain.gain.setValueAtTime(0,time);gain.gain.linearRampToValueAtTime(v*0.48,time+attack);gain.gain.exponentialRampToValueAtTime(sustain,time+Math.max(attack+0.002,Math.min(duration,t.decay)));gain.gain.setValueAtTime(sustain,time+duration);gain.gain.exponentialRampToValueAtTime(0.0001,end);
 const layers=LAYERS[t.instrument]||LAYERS.lead;
...
export class Transport{
 ...
 const time=this.startAt+stepTime(this.next,p);if(time>=this.ctx.currentTime-0.025){const at=this.next;for(const e of eventsAt(p,at,this.mode,this.pattern))if(!this.silenced.has(e.track.id))this.graph.trigger(e.track,e.event,time+e.offset,e.duration);if(this.metronome&&at%4===0)this.click(time,at%16===0);this.queue.push({time,absolute:at,...locateStep(p,at,this.mode,this.pattern)});}this.next++;}
 ...
 // Silencing drops a track from the scheduler only. It is not a project edit, and it leaves
 // live pads and previews on that track audible — the point is to play the part yourself.
 silence(ids=[]){this.silenced=new Set(ids);}
}
